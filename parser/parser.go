...
func (p *Parser) parseExpressionList(edge token.TokenType) []ast.Expression {
	list := []ast.Expression{}

	if p.peekTokenIs(edge) {
		p.nextToken()
		return list
	}
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		// peekToken needs to be either a rbrace or a comma, or it's a invalid exp
...
func (p *Parser) parseIndexExpression(left ast.Expression) ast.Expression {
	exp := &ast.IndexExpresssion{Token: p.currentToken, Left: left}
